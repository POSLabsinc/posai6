...
import { X, Plus, RotateCcw, Check, AlertTriangle, Clock, User, Phone, Mail, ShoppingBag, DollarSign, Heart, FileText, Utensils } from "lucide-react";
...
const GuestPastOrderPopup = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div
        className="relative flex flex-col rounded-2xl shadow-2xl border border-[hsl(var(--border))] overflow-hidden"
        style={{
          width: 580,
          maxWidth: "94vw",
          height: "82%",
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[hsl(var(--border))]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[hsl(25,95%,53%/0.15)] flex items-center justify-center">
              <User className="w-5 h-5 text-[hsl(25,95%,53%)]" />
            </div>
            <div>
              <h2 className="text-base font-semibold leading-tight">{guest.name}</h2>
              <p className="text-xs text-[hsl(var(--muted-foreground))]">Order History</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Guest Info Bar */}
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 px-6 py-3 border-b border-[hsl(var(--border))] bg-[hsl(var(--accent)/0.3)]">
          {guest.phone && (
          ...
          {guest.totalSpent !== undefined && guest.totalSpent > 0 && (
            <span className="flex items-center gap-1.5 text-xs text-[hsl(var(--muted-foreground))]">
              <DollarSign className="w-3 h-3" /> ${guest.totalSpent.toFixed(2)} spent
            </span>
          )}
          ...
          {guest.loyaltyTier && (
            <span className="ml-auto text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[hsl(25,95%,50%/0.15)] text-[hsl(25,95%,45%)]">
              {guest.loyaltyTier}
            </span>
          )}
        </div>

        {/* Allergies & Notes strip */}
        {(hasAllergies || hasNotes) && (
          <div className="flex flex-wrap items-start gap-x-6 gap-y-2 px-6 py-2.5 border-b border-[hsl(var(--border))] bg-[hsl(var(--destructive)/0.05)]">
            {hasAllergies && (
              <div className="flex items-start gap-1.5">
                <AlertTriangle className="w-3.5 h-3.5 mt-0.5 text-[hsl(var(--destructive))] shrink-0" />
                <div>
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-[hsl(var(--destructive))]">Allergies</span>
                  <p className="text-xs text-[hsl(var(--muted-foreground))]">{guest.allergies!.join(", ")}</p>
                </div>
              </div>
            )}
            {hasNotes && (
              <div className="flex items-start gap-1.5">
                <FileText className="w-3.5 h-3.5 mt-0.5 text-[hsl(var(--muted-foreground))] shrink-0" />
                <div>
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-[hsl(var(--muted-foreground))]">Notes</span>
                  <p className="text-xs text-[hsl(var(--muted-foreground))]">{guest.notes}</p>
                </div>
              </div>
            )}
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[hsl(var(--muted-foreground))]">x{item.quantity}</span>
                      </div>
                    ...
                    <div className="text-right shrink-0">
                      {comped ? (
                        <span className="text-xs font-medium text-[hsl(var(--muted-foreground))]">$0.00</span>
                      ) : priceChanged ? (
        ...
        {/* Sticky Footer */}
        <div className="flex items-center gap-3 px-6 py-4 border-t border-[hsl(var(--border))]">
          <button
            onClick={onClose}
