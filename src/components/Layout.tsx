...
export function Layout({
  ...
  return <div className="h-screen flex flex-col w-full overflow-hidden bg-black" onClick={() => isHeaderVisible && setIsHeaderVisible(false)}>
      {/* Header - Hidden by default on mobile, shown when toggled */}
      <div className={`${isHeaderVisible ? 'block' : 'hidden'} md:block flex-shrink-0`} onClick={e => e.stopPropagation()} onTouchStart={handleHeaderTouchStart} onTouchEnd={handleHeaderTouchEnd}>
        <Header />
      </div>
      
      {/* Mobile Header Toggle - Only shown when header is hidden */}
      {!isHeaderVisible && <button onClick={e => {
      e.stopPropagation();
