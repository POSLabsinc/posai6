...
const defaultTables: TableType[] = [
  { id: "T1", seats: 8, status: "Reserved", time: "", shape: "circle", occupiedSeats: [], guests: 0, x: 80, y: 60 },
...
const FloorPlanCircularTable = ({ table, isSelected, isHighlighted }: { 
  ...
  const tableRadius = table.seats >= 8 ? 36 : table.seats >= 6 ? 30 : 24;
    ...
    >
      ...
      >
        ...
        >
          ...
          <span className="text-[8px] font-medium mt-0.5" style={{ color: config.hexColor }}>
            {config.label}
          </span>
        ...
        {table.time && (
          <div 
            className="absolute -bottom-0 left-1/2 -translate-x-1/2 flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-medium"
            style={{ backgroundColor: 'rgba(23, 23, 23, 0.95)', border: `1px solid ${config.hexColor}50` }}
          >
            <Clock className="w-2 h-2" style={{ color: config.hexColor }} />
