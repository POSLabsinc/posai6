 ...
 interface AddDiscountContentProps {
   onBack: () => void;
   onSave: (discount: {
     name: string;
     amount: number;
 ...
 const AddDiscountContent = ({ onBack, onSave }: AddDiscountContentProps) => {
   const [name, setName] = useState("");
   const [amount, setAmount] = useState("");
   const [type, setType] = useState<"Percentage" | "Fixed" | "">("");
   const [applicableTo, setApplicableTo] = useState("");
   const [requiresManagerPin, setRequiresManagerPin] = useState(false);
   ...
   return (
     <div className="h-full flex flex-col overflow-hidden bg-background">
       ...
       <div className="flex items-center justify-center py-4 px-4 relative">
         ...
         <h1 className="text-lg font-semibold text-foreground">Add Discount</h1>
       </div>
