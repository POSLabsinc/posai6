...
const AddTaxContent = ({ onBack, onSave }: AddTaxContentProps) => {
  const [name, setName] = useState("");
  const [amount, setAmount] = useState("");
  const [type, setType] = useState<"Exclusive" | "Inclusive" | "">("");
  ...
  const handleBack = () => {
    ...
    if (name && amount && type) {
      onSave({
        ...
        type: type as "Exclusive" | "Inclusive",
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-center py-4 px-4 relative">
        ...
        <h1 className="text-lg font-semibold text-foreground">Add Tax</h1>
      </div>
