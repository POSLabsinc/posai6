...
const AddTimedPricingRuleContent = ({ onBack, onSave, editRule }: AddTimedPricingRuleContentProps) => {
  ...
  const handleSave = () => {
    ...
    let rules: TimedPricingRule[] = [];
    try { rules = stored ? JSON.parse(stored) : []; } catch {}
      ...
      {isMobile ? (
        <>
          <AppleWheelTimePicker
            ...
            onConfirm={(time) => { setStartTime(time); setShowStartTimePicker(false); }}
            selectedTime={startTime}
          />
          <AppleWheelTimePicker
            isOpen={showEndTimePicker}
            onClose={() => setShowEndTimePicker(false)}
            onConfirm={(time) => { setEndTime(time); setShowEndTimePicker(false); }}
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-between py-4 px-4 relative">
        ...
        <button
          onClick={handleSave}
          className="px-4 py-2 rounded-full bg-primary text-primary-foreground text-sm font-medium active:opacity-70 transition-opacity"
        >
          Save
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pt-0 px-6 pb-28">
          ...
          <div className="grid grid-cols-2 gap-3">
            {typeOptions.map((opt) => (
              <button
                ...
                style={
                  type === opt.value ? { boxShadow: `0 0 0 2px ${opt.color}` } : {}
                }
              >
                <div
        ...
        <p className="text-neutral-500 text-sm px-1 mt-1.5 mb-6">
          Select the type of pricing rule. Happy Hour applies discounts, Peak Time adds surcharges, and Late Night or Custom lets you define any adjustment.
        </p>
        ...
        <p className="text-neutral-500 text-sm px-1 mt-1.5 mb-6">
          Give this rule a descriptive name so you can quickly identify it in the pricing rules list.
        </p>
        ...
        <div className="mb-1">
          ...
          <div className="bg-neutral-800/60 rounded-2xl overflow-hidden">
            ...
            <button
              ...
              className="flex items-center justify-between w-full py-3.5 px-4 border-b border-neutral-700/50 active:opacity-70 transition-opacity"
            >
        ...
        <p className="text-neutral-500 text-sm px-1 mt-1.5 mb-6">
          Set the time window during which this pricing rule will be active. Product prices will automatically adjust within this period.
        </p>
        ...
        <p className="text-neutral-500 text-sm px-1 mt-1.5 mb-6">
          Choose which days of the week this pricing rule applies. Tap a day to toggle it on or off.
        </p>
            ...
            <div className="flex items-center justify-between py-3.5 px-4">
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ color: adjustment < 0 ? "#34C759" : adjustment > 0 ? "#FF9500" : undefined }}
                >
                  {adjustment > 0 ? "+" : ""}{adjustment}%
                </span>
                <span className="text-neutral-500 text-sm">
                  {adjustment < 0 ? "Discount" : adjustment > 0 ? "Surcharge" : "No Change"}
                </span>
              </div>
              <div className="flex items-center bg-neutral-700/50 rounded-lg overflow-hidden">
                <button
                  onClick={() => setAdjustment(Math.max(-100, adjustment - 5))}
                ...
                <button
                  onClick={() => setAdjustment(Math.min(100, adjustment + 5))}
        ...
        <p className="text-neutral-500 text-sm px-1 mt-1.5 mb-6">
          Set the percentage adjustment applied to product prices. Use negative values for discounts (e.g., -20% for Happy Hour) or positive values for surcharges (e.g., +15% for Peak Time). Adjust in 5% increments.
        </p>
          ...
          <div className="bg-neutral-800/60 rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-4">
              <div
                ...
                style={{ backgroundColor: selectedTypeOption.color + "22" }}
              >
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-muted-foreground">
                {days.length === 7 ? "Every day" : days.length === 0 ? "No days" : days.join(", ")}
              </span>
              <span
                className="font-medium"
                style={{ color: adjustment < 0 ? "#34C759" : "#FF9500" }}
              >
                {adjustment > 0 ? "+" : ""}{adjustment}%
              </span>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-neutral-700/50">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-foreground font-semibold text-base">
                  £{(10 + (10 * adjustment / 100)).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
