...
const defaultModifiersList: DefaultModifier[] = [
  { id: "1", name: "Almonds", type: "Exceptional", archived: false },
  { id: "2", name: "Chamomile Tea", type: "Normal", archived: false },
  { id: "3", name: "Eggs (2)", type: "Exceptional", archived: false },
  { id: "4", name: "Extra Kaju", type: "Normal", archived: false },
  { id: "5", name: "Extra Oil", type: "Normal", archived: false },
  { id: "6", name: "Extra Rice", type: "Normal", archived: false },
  { id: "7", name: "Extra Vegetables", type: "Normal", archived: false },
  { id: "8", name: "Less Kaju", type: "Normal", archived: false },
  { id: "9", name: "Less Oil", type: "Normal", archived: false },
  { id: "10", name: "Less Rice", type: "Normal", archived: false },
];
...
const DefaultModifiersContent = ({ showHeader = true, onBack, onAIClick }: DefaultModifiersContentProps) => {
  ...
  const [modifiers, setModifiers] = useState<DefaultModifier[]>(() => {
    ...
    if (stored) {
      try {
        ...
        console.error("Failed to parse default modifiers from localStorage", e);
      }
    }
    return defaultModifiersList;
  });
  ...
  const saveModifiers = (newItems: DefaultModifier[]) => {
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-6 pb-6">
          ...
          <section className="mt-4 rounded-[28px] bg-[#26262699] px-10 py-8 text-center">
            <div 
              ...
              style={{ backgroundColor: getIconBgColor("#48009E") }}
            >
              <img src={defaultModifiersIcon} alt="Default Modifiers" className="h-8 w-8 object-contain" />
            </div>
            <h1 className="text-2xl font-semibold leading-tight text-foreground">Default Modifiers</h1>
            <p className="mx-auto mt-2 max-w-3xl text-[15px] leading-relaxed text-[hsl(var(--text-subtle))]">
              Set up modifiers that are automatically applied to menu items by default.
            </p>
          </section>
          ...
          <section className="mt-6 flex items-center gap-2 lg:gap-4">
            ...
            <button
              onClick={() => navigate('/settings/menu/default-modifiers/add')}
              ...
              filteredItems.map((item, index) => (
                <div key={item.id}>
                  ...
                  <button onClick={() => navigate(`/settings/menu/default-modifiers/edit/${item.id}`)} className="grid grid-cols-[1.2fr_1fr_24px] items-center px-8 py-5 w-full hover:bg-neutral-700/30 transition-colors text-left">
                    ...
                    <span className="text-[15px] text-foreground">{item.type}</span>
      ...
      {showHeader && (
        <div className="flex items-center justify-center py-4 px-4 relative">
          ...
          <div className="flex items-center gap-1">
            <h1 className="text-lg font-semibold text-foreground">Default Modifiers</h1>
            <button
              onClick={() => {
                toast({
                  description: "Set up modifiers that are automatically applied to menu items by default.",
                  duration: 4000,
                });
              }}
            ...
            filteredItems.map((item, index) => (
              <div key={item.id}>
                ...
                <SwipeableSettingsItem
                  onTap={() => navigate(`/settings/menu/default-modifiers/edit/${item.id}`)}
                  onArchive={() => handleArchiveItem(item)}
                  isArchived={item.archived}
                >
                  <div className="grid grid-cols-[1fr_1fr_24px] items-center w-full py-4 px-4">
                    <span className="text-foreground text-base font-medium text-left">{item.name}</span>
                    <span className="text-foreground text-base text-left">{item.type}</span>
