 ...
 interface EditMenuItemContentProps {
   item: MenuItem;
   onBack: () => void;
 ...
 const EditMenuItemContent = ({ item, onBack, onSave }: EditMenuItemContentProps) => {
   const [name, setName] = useState(item.name);
   const [enabled, setEnabled] = useState(item.enabled);
   const [pointOfSale, setPointOfSale] = useState(item.pointOfSale ?? true);
   const [pointOfPurchase, setPointOfPurchase] = useState(item.pointOfPurchase ?? false);
   const [selfServiceKiosk, setSelfServiceKiosk] = useState(item.selfServiceKiosk ?? false);
   const [onlineOrders, setOnlineOrders] = useState(item.onlineOrders ?? false);
   const [selectedCategories, setSelectedCategories] = useState<string[]>(
     item.operationCategories ? item.operationCategories.split(", ").filter(Boolean) : []
   );
   const [selectedRevenueCenters, setSelectedRevenueCenters] = useState<string[]>(
     item.revenueCenters ? item.revenueCenters.split(", ").filter(Boolean) : []
   );
           ...
           <div className="py-4 px-4">
             <div className="flex items-center justify-between">
               ...
               <div className="flex items-center gap-2">
                 <span className="text-neutral-400 text-sm">{enabled ? "Active" : "Inactive"}</span>
