...
interface EditServiceChargeContentProps {
  serviceCharge: ServiceCharge;
  onBack: () => void;
  onSave: (serviceCharge: ServiceCharge) => void;
}
...
const EditServiceChargeContent = ({ serviceCharge, onBack, onSave }: EditServiceChargeContentProps) => {
  const isMobile = useIsMobile();
  const [name, setName] = useState(serviceCharge.name);
  const [amount, setAmount] = useState(serviceCharge.amount.toString());
  const [type, setType] = useState<"Percentage" | "Fixed">(serviceCharge.type);
  const [taxApplicable, setTaxApplicable] = useState(serviceCharge.taxApplicable || "");
  const [orderTypes, setOrderTypes] = useState<string[]>(() => {
    const ot = serviceCharge.orderType;
    if (Array.isArray(ot)) return ot;
    if (typeof ot === "string" && ot) return [ot];
    return [];
  });
  const [appliedAs, setAppliedAs] = useState(serviceCharge.appliedAs || "Basic");
  const [automaticApply, setAutomaticApply] = useState(serviceCharge.automaticApply || false);
  const [minSeats, setMinSeats] = useState(serviceCharge.minSeats || 0);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-center py-4 px-4 relative">
        ...
        <h1 className="text-xl font-semibold text-foreground">Edit Service Charge</h1>
      </div>
...
export default EditServiceChargeContent;
