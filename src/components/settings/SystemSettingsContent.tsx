...
const SystemSettingsContent = ({
  ...
  return <div className="h-full overflow-y-auto scrollbar-hide overscroll-contain">
      {showHeader && <div className="flex items-center py-4 relative md:hidden px-4">
      ...
      <div className={`${showHeader ? 'pt-0' : 'pt-0'} px-4 md:px-6 pb-28`}>
        ...
        <div className="bg-neutral-800/60 rounded-full overflow-hidden mb-1.5">
          <SettingsOption icon={aiIntegrationIcon} iconBgColor="#3B82F6" label="AI Integration" onClick={() => onNavigate?.('/settings/network/ai-integration')} showDivider={false} />
        </div>
        <p className="text-xs text-neutral-500 px-1 mb-4">
