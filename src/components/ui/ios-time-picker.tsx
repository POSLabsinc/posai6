  ...
  ({ value, onChange, className }, ref) => {
    ...
    const PickerColumn = ({
      ...
      <div className="relative flex-1 h-[200px]">
        ...
        {/* Selection highlight */}
        <div className="absolute top-1/2 left-1 right-1 h-10 -translate-y-1/2 bg-gray-100 rounded-lg z-0" />
        ...
        >
          ...
          {items.map((item, idx) => (
            <div
              key={idx}
              className={cn(
                "h-10 flex items-center justify-center text-xl font-medium snap-center cursor-pointer transition-all",
                selected === item ? "text-black" : "text-gray-400"
              )}
              onClick={() => {
                onSelect(item);
    ...
    return (
      <div ref={ref} className={cn("flex bg-white rounded-xl p-2 gap-1", className)}>
        <PickerColumn
          items={hours}
