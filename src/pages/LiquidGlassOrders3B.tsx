...
const categoryGradients: Record<string, string> = {
  "Starters": "from-emerald-500/20 to-teal-500/10",
  "Mains": "from-orange-500/20 to-red-500/10",
  "Sides": "from-amber-500/20 to-yellow-500/10",
  "Desserts": "from-pink-500/20 to-rose-500/10",
  "Drinks": "from-blue-500/20 to-cyan-500/10",
  "Specials": "from-purple-500/20 to-violet-500/10",
};

const LiquidGlassOrders3B = () => {
  ...
  const currentGradient = categoryGradients[activeCategory] || "from-orange-500/20 to-amber-500/10";

  return (
    <div className="flex flex-col overflow-hidden relative" style={{ height: 'calc(100vh - 60px)' }}>
      {/* Dynamic Gradient Background */}
      <div className={`absolute inset-0 bg-gradient-to-br ${currentGradient} transition-all duration-700`} />
      <div className="absolute inset-0 bg-neutral-950/80" />
      
      <div className="relative flex flex-col flex-1 overflow-hidden">
        {/* Floating Tab Bar at Top Center (iOS style) */}
        <div className="flex-shrink-0 p-4 flex justify-center">
          <div className="glass-glow rounded-full p-1.5 flex items-center gap-1 max-w-fit">
            <Select value={selectedMenu} onValueChange={setSelectedMenu}>
              <SelectTrigger className="w-[140px] rounded-full bg-white/10 border-0 text-white h-9 text-sm">
        ...
        {/* Subcategories - Morphing Pills Below */}
        ...
        <div className="flex flex-1 gap-4 px-4 pb-4 overflow-hidden">
          {/* Main Content - Overlapping Glass Cards */}
          <div className="flex-1 overflow-hidden">
            <div className="mb-3 text-center">
              <p className="text-sm text-white/40">Liquid Glass 3B: Floating Tab Bar</p>
            </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {menuItems.map((item, index) => (
                  <div
                    ...
                    className="glass-vibrant rounded-2xl overflow-hidden hover:scale-[1.03] hover:-translate-y-1 transition-all duration-300 cursor-pointer group animate-fade-in"
                    style={{ 
                      animationDelay: `${index * 50}ms`,
                      transform: index % 2 === 0 ? 'translateY(10px)' : 'translateY(0)'
                    }}
                  >
          ...
          {/* Right Panel - Persistent Bottom Glass Sheet */}
...
export default LiquidGlassOrders3B;