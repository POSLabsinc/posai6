...
const categoryColors: Record<string, string> = {
  "Starters": "from-emerald-500 to-teal-400",
  "Mains": "from-orange-500 to-red-400",
  "Sides": "from-amber-500 to-yellow-400",
  "Desserts": "from-pink-500 to-rose-400",
  "Drinks": "from-blue-500 to-cyan-400",
  "Specials": "from-purple-500 to-violet-400",
};

const LiquidGlassOrders4A = () => {
    ...
    <div className="flex gap-4 overflow-hidden bg-neutral-950 gradient-mesh p-4" style={{ height: 'calc(100vh - 60px)' }}>
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-white/40 text-sm">Liquid Glass 4A: Glass Category Cards</span>
        </div>

        {/* Main Category Cards - Large Glass Vibrant Tiles */}
        <div className="flex-shrink-0">
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {menuCategories[selectedMenu]?.map((cat, index) => {
              const itemCount = categorySubcategories[cat]?.length || 0;
              const gradientColor = categoryColors[cat] || "from-orange-500 to-amber-400";
              return (
                <div
                  key={cat}
                  onClick={() => {
                  ...
                  className={`relative flex-shrink-0 w-36 h-28 rounded-2xl cursor-pointer transition-all duration-300 hover:scale-105 animate-fade-in ${
                    activeCategory === cat 
                      ? "glass-glow ring-2 ring-white/30" 
                      : "glass-vibrant"
                  }`}
                  style={{ animationDelay: `${index * 60}ms` }}
                >
                  {/* Gradient Orb */}
                  <div className={`absolute top-2 right-2 w-10 h-10 rounded-full bg-gradient-to-br ${gradientColor} opacity-60 blur-lg`} />
                  
                  <div className="relative h-full p-4 flex flex-col justify-between">
                    <span className="text-base font-bold text-white">{cat}</span>
                    <span className="text-xs text-white/50">{itemCount} options</span>
                  </div>
                  
                  {/* Floating Badge */}
                  <div className={`absolute -top-2 -right-2 w-7 h-7 rounded-full bg-gradient-to-r ${gradientColor} text-white text-xs font-bold flex items-center justify-center shadow-lg`}>
                    {itemCount}
                  </div>
                </div>
              );
        ...
        {/* Subcategory Pills - Horizontal Scrolling */}
        <div className="flex-shrink-0">
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {(categorySubcategories[activeCategory] || []).map((sub, index) => (
              <button
                ...
                className={`px-5 py-3 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-300 animate-scale-in ${
        ...
        {/* Menu Items Grid - 3-Column Glass Grid */}
      ...
      {/* Right Panel - Order with Glass Summary Cards */}
...
export default LiquidGlassOrders4A;