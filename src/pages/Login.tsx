              ...
              <div className="flex-1 px-6">
                ...
                <AnimatePresence mode="wait">
                  {!existingUserCodeSent ? (
                    <motion.div key="email-step" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <h2 className="text-lg font-bold text-foreground mb-2">Sign in with Code</h2>
                      <p className="text-sm text-foreground/50 mb-5">Enter your email or mobile number to receive a code</p>
                      <div className="space-y-3">
                        <div className="relative" data-tour="email-input-field">
                          <div className="absolute left-4 top-1/2 -translate-y-1/2 text-foreground/30"><Mail className="w-5 h-5" /></div>
                    ...
                    <motion.div key="otp-step" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <h2 className="text-lg font-bold text-foreground mb-2">Enter Code</h2>
                      <p className="text-sm text-foreground/50 mb-5">Enter the 6-digit code sent to <span className="font-semibold text-foreground">{existingUserContact}</span></p>
                      <div className="flex gap-1.5 mb-4" data-tour="otp-code-area">
                        {Array.from({ length: 6 }).map((_, i) => (
                          <div key={i} className={`w-9 h-11 rounded-lg border-2 flex items-center justify-center text-lg font-bold transition-all ${existingUserVerificationCode[i] ? "border-primary bg-primary/5 text-foreground" : i === existingUserVerificationCode.length ? "border-primary/50 bg-foreground/[0.03]" : "border-foreground/10 bg-foreground/[0.03]"}`}>
