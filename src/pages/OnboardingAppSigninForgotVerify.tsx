...
const STORAGE_KEY = "onboarding_signin_forgot_verify_state";
...
const OnboardingAppSigninForgotVerify = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const isLandscape = useIsLandscape();
  const incoming = (location.state as LocationState) ?? {};
  const persisted = readPersisted();
  const hasIncoming = !!incoming.email;
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      navigate("/onboarding/app/signin/forgot/reset", { state: { email } });
    } finally {
      setSubmitting(false);
    }
  };
...
export default OnboardingAppSigninForgotVerify;
