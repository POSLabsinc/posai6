...
const Orders = () => {
  ...
  return <div className="flex flex-col md:flex-row gap-1 md:gap-3 lg:gap-4 h-full overflow-hidden pb-16 md:pb-0 relative">
      ...
      {/* Left Panel - Menu */}
      <div className={`md:flex-1 flex flex-col min-w-0 bg-neutral-900 md:bg-black border-t border-sidebar-border md:border-0 rounded-t-[20px] md:rounded-none absolute md:relative md:bottom-auto md:left-0 md:right-0 md:left-auto md:right-auto z-10 bottom-[calc(2.5rem)] left-0 right-0 overflow-hidden ${!isDragging ? 'transition-all duration-300 ease-out' : ''} ${menuPosition === 'minimized' ? 'h-12' : menuPosition === 'center' ? 'h-[calc(100vh-21.5rem)]' : 'h-[calc(100vh-10.5rem)]'} md:h-auto md:top-auto md:bottom-auto`} style={isDragging && dragOffset !== 0 ? {
