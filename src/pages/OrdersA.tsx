...
import { Plus, ChevronDown, X, Printer, ShoppingCart, ChevronUp } from "lucide-react";
...
const OrdersA = () => {
  ...
  // Order Panel Content - reusable for both desktop sidebar and mobile drawer
    ...
    <>
      ...
      <div className="p-3 border-b border-neutral-700">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="bg-neutral-800 border-neutral-700 z-[100]">
            {orderTypes.map(type => (
              <DropdownMenuItem 
                key={type} 
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="lg:hidden bg-neutral-900 rounded-xl p-2 m-1 md:m-2 flex-shrink-0">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
          {foodTruckCategories.map(category => (
            <Button
              ...
              className={`rounded-full px-3 sm:px-4 h-9 text-xs whitespace-nowrap border-2 flex-shrink-0 ${
      ...
      {/* Main Content Area */}
      <div className="flex flex-1 gap-1 md:gap-2 overflow-hidden px-1 md:px-2 pb-1 md:pb-2">
        {/* Left Sidebar - Categories (Desktop Only) */}
        <div className="hidden lg:flex flex-col bg-neutral-900 rounded-xl w-48">
          <div className="p-3 border-b border-neutral-700">
            <h2 className="text-white font-bold text-sm">FOOD TRUCK</h2>
          </div>
        ...
        <div className="flex-1 flex flex-col bg-neutral-900 rounded-xl overflow-hidden">
          ...
          <div className="p-2 sm:p-3 border-b border-neutral-700">
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
              {(categorySubcategories[activeCategory] || []).map(sub => (
                <Button
                  key={sub}
                  variant="outline"
                  className={`rounded-md px-3 h-7 text-[10px] sm:text-xs whitespace-nowrap border ${
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3">
              {menuItems
                ...
                .map((item, index) => (
                  <div
                    key={item.id}
                    className="bg-neutral-800 rounded-xl overflow-hidden border border-neutral-700 hover:border-orange-500 transition-all cursor-pointer shadow-lg hover:shadow-xl hover:shadow-orange-500/10 active:scale-[0.98]"
                  ...
                  >
                    <div className="relative aspect-square sm:aspect-video">
                      ...
                      <button
                        ...
                        className="absolute top-2 left-2 w-8 h-8 bg-orange-500 hover:bg-orange-600 active:bg-orange-700 rounded-full flex items-center justify-center shadow-lg transition-transform hover:scale-110 active:scale-95"
                      >
                    ...
                    >
                      <h3 className="text-xs sm:text-sm font-medium text-white line-clamp-2">{item.name}</h3>
                      <p className="text-orange-500 font-semibold text-sm sm:text-base mt-0.5">${item.price.toFixed(2)}</p>
                    </div>
                  </div>
                ))}
            </div>
        ...
        {/* Right Panel - Order (Tablet & Desktop) */}
        <div className="hidden md:flex w-64 lg:w-80 flex-col bg-neutral-900 rounded-xl overflow-hidden">
          <OrderPanelContent />
        </div>
      </div>

      {/* Mobile Cart Button - Fixed at Bottom */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 p-2 bg-neutral-950/95 backdrop-blur-sm border-t border-neutral-800 z-40">
        <Drawer open={mobileCartOpen} onOpenChange={setMobileCartOpen}>
          <DrawerTrigger asChild>
            <button 
              className="w-full h-14 rounded-xl flex items-center justify-between px-4 shadow-lg"
              style={{ background: 'linear-gradient(180deg, #FF6B35 0%, #D44A1A 100%)' }}
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-white font-semibold">
                  {orderItems.length === 0 ? 'View Order' : `${orderItems.length} item${orderItems.length > 1 ? 's' : ''}`}
                </span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <DrawerContent className="bg-neutral-900 border-neutral-700 max-h-[85vh]">
            <div className="flex flex-col h-full max-h-[85vh]">
              <div className="w-12 h-1.5 bg-neutral-600 rounded-full mx-auto my-3 flex-shrink-0" />
              <OrderPanelContent />
            </div>
          </DrawerContent>
        </Drawer>
      ...
      {/* Spacer for mobile to prevent content being hidden behind fixed cart button */}
