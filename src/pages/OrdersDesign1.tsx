...
import { Plus, Save, Flame, Receipt, FileText, ChevronDown, ChevronRight } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
const OrdersDesign1 = () => {
  ...
  return (
    <div className="flex gap-3 overflow-hidden" style={{ height: 'calc(100vh - 60px)' }}>
      {/* Left Panel - Accordion Categories */}
      <div className="w-72 flex flex-col bg-neutral-900 rounded-lg overflow-hidden flex-shrink-0">
        {/* Menu Selector */}
        <div className="p-3 border-b border-sidebar-border">
          <div className="flex items-center gap-2">
            ...
            <Select value={selectedMenu} onValueChange={setSelectedMenu}>
              <SelectTrigger className="flex-1 rounded-full border-sidebar-border bg-background text-foreground h-10">
          ...
          >
            {menuCategories[selectedMenu].map((cat) => (
              <AccordionItem key={cat} value={cat} className="border-b-0 mb-1">
                <AccordionTrigger 
                  className={`rounded-xl px-4 py-3 text-base font-bold hover:no-underline transition-all ${
                    expandedCategory === cat 
                      ? "bg-orange-500 text-white [&>svg]:text-white" 
                ...
                <AccordionContent className="pt-2 pb-0">
                  <div className="flex flex-wrap gap-2 pl-2">
                    {(categorySubcategories[cat] || []).slice(0, 8).map((sub) => (
                      <Button
                        key={sub}
                        variant="outline"
                        onClick={() => setActiveSubcategory(sub)}
                        className={`rounded-full px-4 py-2 h-auto min-h-[40px] text-sm font-medium text-center whitespace-normal leading-tight max-w-[130px] border ${
      ...
      <div className="flex-1 flex flex-col bg-neutral-900 rounded-lg p-3 min-w-0">
        <div className="mb-3">
          <h2 className="text-lg font-bold text-foreground">
            {activeCategory} {activeSubcategory && `› ${activeSubcategory}`}
          </h2>
          <p className="text-sm text-muted-foreground">Design 1: Vertical Accordion</p>
        </div>
