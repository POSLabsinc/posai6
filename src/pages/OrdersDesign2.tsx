  ...
  return (
    <div className="flex gap-3 overflow-hidden" style={{ height: 'calc(100vh - 60px)' }}>
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 overflow-hidden bg-neutral-900 rounded-lg">
        {/* ROW 1: Main Categories - Fixed height, horizontal scroll */}
        <div className="flex-shrink-0 border-b-4 border-orange-500/30 bg-neutral-800/50">
          <div className="flex items-center gap-2 p-3 overflow-x-auto scrollbar-hide">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              >
                <img src={isMenuSelectOpen ? burgerCloseIcon : burgerOpenIcon} alt="Menu" className="w-8 h-8" />
              </Button>
              {isMenuSelectOpen && (
                <Select value={selectedMenu} onValueChange={setSelectedMenu}>
                  <SelectTrigger className="w-[160px] rounded-xl border-sidebar-border bg-background text-foreground h-14">
            ...
            {menuCategories[selectedMenu].map((cat) => (
              <Button
                ...
                className={`rounded-xl min-w-[120px] h-14 px-6 text-base font-bold whitespace-nowrap flex-shrink-0 transition-all ${
                  activeCategory === cat 
                    ? "bg-orange-500 hover:bg-orange-600 text-white border-b-4 border-orange-700 shadow-lg shadow-orange-500/30" 
        ...
        {/* ROW 2: Subcategories - Fixed height, horizontal scroll */}
        <div className="flex-shrink-0 bg-neutral-800/30 border-b border-sidebar-border">
          <div className="flex items-start gap-2 p-3 overflow-x-auto scrollbar-hide">
            {(categorySubcategories[activeCategory] || []).map((sub) => (
              <Button
                key={sub}
                variant="outline"
                onClick={() => setActiveSubcategory(sub)}
                className={`rounded-full px-6 py-3 h-auto min-h-[44px] max-w-[160px] text-sm font-medium flex-shrink-0 whitespace-normal text-center leading-tight border transition-all ${
                  activeSubcategory === sub 
                    ? "bg-amber-400 hover:bg-amber-500 text-black border-amber-400 shadow-md shadow-amber-400/30" 
                    : "bg-header text-header-foreground border-white/40 hover:bg-header/80"
                }`}
              >
                {sub}
              </Button>
        ...
        <div className="px-3 py-2 flex-shrink-0">
          <p className="text-sm text-muted-foreground">
            Design 2: Two-Row Horizontal Tabs • <span className="text-foreground font-medium">{activeCategory}</span> › <span className="text-amber-400">{activeSubcategory}</span>
          </p>
        </div>
