...
import { Search, SlidersHorizontal, X, Phone, Clock, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, GripVertical, Printer, ArrowDownUp, Check, Package, Plus, CalendarDays, Info } from "lucide-react";
...
// Scheduled orders get proper future scheduled times
const mockOrders: OnlineOrder[] = mockOrdersData.map((order, index) => {
  // Handle scheduled orders separately
  if (order.isScheduled) {
    // Generate realistic scheduled times in the future
    const scheduledOffsets = [30, 90, 120, 180, 240, 300, 360, 420, 480, 540, 600];
    const offsetMinutes = scheduledOffsets[index % scheduledOffsets.length];
    const deliveryOffset = order.orderType === 'DELIVERY' ? 15 : undefined;
    
    const scheduledTimes = generateScheduledOrderTimes(offsetMinutes, 30, deliveryOffset);
    
    return {
      ...order,
      orderedAt: scheduledTimes.orderedAt,
      orderedAtTimestamp: scheduledTimes.orderedAtTimestamp,
      scheduledFor: scheduledTimes.scheduledFor,
      estimateReady: scheduledTimes.estimateReady,
      deliverBy: scheduledTimes.deliverBy || order.deliverBy,
      isOverdue: false,
      isWaiting: true
    };
  }
...
const defaultDayParts: DayPart[] = [
  ...
  { id: 'EVENING_SNACKS', label: 'Evening Snacks', startHour: 15, startMinute: 0, endHour: 18, endMinute: 0, icon: '🍿', emptyText: 'No evening snack orders', priority: 4 },
...
const ScheduledOrdersV3 = () => {
  ...
  const handleReportItems = useCallback((items: ReportedItem[]) => {
    if (orderToReport) {
  ...
  const platformIconsMap = useMemo(() => {
    ...
    return map;
  }, [orders.map(o => `${o.id}-${o.platform}`).join(',')]);
  ...
  const KanbanColumn = useCallback(({ column }: { column: { id: string; label: string; emptyText: string; emptyIcon: string; description?: string; dayPart?: DayPart } }) => {
    ...
    const getDayPartHeaderStyles = (columnId: string, dayPart?: DayPart) => {
      ...
      const dayPartColors: Record<string, { primary: string; rgba: string }> = {
        'BREAKFAST': { primary: '#FFB347', rgba: '255, 179, 71' }, // Warm orange - morning sun
        'BRUNCH': { primary: '#FFCC80', rgba: '255, 204, 128' }, // Light orange - mid-morning
        'LUNCH': { primary: '#FFD54F', rgba: '255, 213, 79' }, // Golden yellow - midday
        'EVENING_SNACKS': { primary: '#FFAB91', rgba: '255, 171, 145' }, // Coral - evening glow
        'DINNER': { primary: '#FF8A80', rgba: '255, 138, 128' }, // Warm red - dinner time
        'LATE_NIGHT': { primary: '#CE93D8', rgba: '206, 147, 216' }, // Purple - night
      };
      
      const colors = dayPartColors[columnId] || { primary: '#FFFFFF', rgba: '255, 255, 255' };
    ...
    return (
      ...
      >
        {/* Column Header */}
        <div 
          className={`flex items-center justify-between px-3 py-2 mb-2 rounded-xl mx-1 ${shouldPulse ? 'animate-pulse' : ''}`}
          style={headerStyles ? { background: headerStyles.background, border: headerStyles.border } : {}}
        >
          <div className="flex items-center gap-2">
            {/* Day-part icon */}
            {column.emptyIcon && activeMode === 'SCHEDULED' && (
              <span className="text-lg">{column.emptyIcon}</span>
            )}
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span 
                ...
                {/* NOW Badge - Current Service Indicator */}
                {headerStyles?.isCurrent && (
                  <span 
                    className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider animate-pulse"
                    style={{ 
                      background: headerStyles.colors?.primary || '#FFB347',
                      color: '#000',
                      boxShadow: `0 0 12px ${headerStyles.colors?.primary || '#FFB347'}80`
                    }}
                  >
                    Now
                  </span>
                )}
                {/* UP NEXT Badge - Next Service Indicator */}
                {headerStyles?.isNext && (
                  <span 
              ...
              {/* Time range display for scheduled day-parts */}
              {column.dayPart && activeMode === 'SCHEDULED' && (
            ...
            <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${headerStyles ? `${headerStyles.countBg} ${headerStyles.countText}` : 'bg-white/20 text-white'}`}>
              {columnOrders.length}
            </span>
          </div>
          <DropdownMenu>
        ...
        <div className="px-4 py-3 border-t border-white/10">
          ...
          {selectedOrder.status === 'READY' && (
            <button 
              ...
              style={{ background: "linear-gradient(180deg, #C2C2C2 0%, #FFFFFF 100%)" }}
            >
              {selectedOrder.orderType === 'DELIVERY' ? 'Dispatch' : 'Ready for Pick Up'}
            </button>
          )}
  ...
  const desktopLayout = (
    <div className="flex h-full bg-black">
      ...
      <div className="flex-1 flex flex-col m-3 mr-1.5 rounded-[20px] overflow-hidden" style={{ background: "linear-gradient(180deg, rgba(78, 78, 78, 0.6) 0%, rgba(62, 62, 62, 0.6) 100%)", boxShadow: "inset 4px 4px 24px 0px rgba(255, 255, 255, 0.15)" }}>
      ...
      {selectedOrder && (
        <div className="w-[380px] flex flex-col m-3 ml-1.5 rounded-[20px] overflow-hidden animate-slide-in-right" style={{ background: "linear-gradient(180deg, rgba(78, 78, 78, 0.6) 0%, rgba(62, 62, 62, 0.6) 100%)", boxShadow: "inset 4px 4px 24px 0px rgba(255, 255, 255, 0.15)" }}>
          <OrderDetailPanel />
        </div>
      )}
    </div>
    ...
    return (
      <>
        ...
        <AppleAlertDialog
          ...
          title="Cancel Order"
          description={`Are you sure you want to cancel Order #${orderToCancel?.orderNumber}? This action cannot be undone.`}
...
export default ScheduledOrdersV3;
